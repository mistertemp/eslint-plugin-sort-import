                    ...
                    if (currentGroupSyntaxGroupIndex !== previousGroupSyntaxGroupIndex) {
                        if (currentGroupSyntaxGroupIndex < previousGroupSyntaxGroupIndex) {
                            context.report({
                                ...
                                fix: fixer => {
                                    fixer.replaceText(previousDeclaration, sourceCode.getText(node));
                                    fixer.replaceText(node, sourceCode.getText(previousDeclaration));
                                }
                            });
                        }
                        if (newLineBetweenGroups && currentGroupSyntaxGroupIndex > previousGroupSyntaxGroupIndex && (previousDeclaration.loc.end.line + 1) >= node.loc.start.line) {
                            context.report({
                                node,
